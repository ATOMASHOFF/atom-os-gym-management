...
export default function SuperDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Total Members',     value: fmt.num(stats?.total_members),     icon: 'members',       color: T.blue,    sub: `${fmt.num(stats?.new_this_month)} new this month`, className: 'fadeUp-2' },
    ...
    { label: 'Today Check-ins',   value: fmt.num(stats?.today_checkins),   icon: 'attendance',    color: T.amber,   sub: `${fmt.num(stats?.total_checkins)} total`, className: 'fadeUp-4' },
    { label: 'Total Revenue',     value: fmt.currency(stats?.total_revenue), icon: 'money',       color: T.purple,  sub: `${fmt.currency(stats?.monthly_revenue)} this month`, className: 'fadeUp-5' },
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Btn onClick={() => navigate('/super/gyms/new')}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 14, marginBottom: 28 }}>
        {loading
          ? Array.from({length: 6}).map((_, i) => <div key={i} className="skeleton" style={{ height: 108, borderRadius: 6 }} />)
        ...
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {gyms.map(g => (
                  <tr key={g.id} className="hover-row" style={{ cursor: 'pointer' }} onClick={() => navigate(`/super/gyms/${g.id}`)}>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ fontWeight: 600 }}>{g.name}</div>
                      <div style={{ fontSize: 11, color: T.muted, fontFamily: T.mono }}>{g.owner_email}</div>
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ background: T.accentDim, color: T.accent, padding: '2px 8px', borderRadius: 3, fontSize: 10, fontFamily: T.mono, textTransform: 'uppercase' }}>{g.plan}</span>
                    </td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <Icon name="chevron_right" size={16} color={T.muted} />
                    </td>
                  </tr>
                ))}
              </tbody>
